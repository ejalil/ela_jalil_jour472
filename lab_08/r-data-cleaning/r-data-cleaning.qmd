...
```{r}
#I would utilize the code that was described in adding a value for Alabama so that the summarization function worked.  
#First I would create a mutate() function that involved case_when() so that the code would know that this is a conditional statement. This would also create a new column that still includes the old values. I am trying to overwrite the state column so that the md is consistent between all of the state values so that all of them are called in a summarization function. Then I would use str_to_title() piped with upper so that all of the values are uppercased. I would use the case_when() function to make it so when the state == Maryland ~ MD, TRUE=state; which would make it so that the Maryland value is switched to MD. All of this would ensure that the state column had MD  which would make the summarization function run properly. 
```
