...
arrange(desc(Diff_Median_PCT))
 #Beaver Creek CDP had the highest absolute change of $118292 between 2010 and 2020. Shaft CDP had the highest percentage change of different median income between 2010 and 2020 of 399.88%. 
```
...
city_md_income %>% 
 ...
 #Danville CDP had the most negative percentage change with -96.33% change in median income. 
```
...
city_md_income %>% 
summary()
#The value of the first quartile for Maryland cities median income for 2020 is 63327. 
```
...
```{r} 
#To determine the median value of the percentage change you would enter median(md_cities$Diff_Median_PCT, na.rm=TRUE) function. Then, I would call city_md_income, and then I would select place, and median income in 2020. Then I would utilize slice max, with a range of 10 to get the top 10 neighborhoods with the highest change. Then I would create another code block and call city_md_income, and then select place, and Diff_Median_PCT. I would then use slice_min with a range of 10 to find the cities with the lowest change. 
```
